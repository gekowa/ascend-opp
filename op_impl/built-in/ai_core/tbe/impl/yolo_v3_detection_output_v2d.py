...
from impl import yolo_v3_cls_prob_v2 as cls
...
def yolo_v3_detection_output_v2d(x, box_out, box_out_num, biases,
...
class DetectionOutput(cls.ClsProbComputer):
    ...
    def __init__(self, input_dict):
        ...
        if tbe_platform.cce_conf.get_soc_spec(
                tbe_platform.cce_conf.UB_SIZE) // 1024 < 200 or \
    ...
    def class_filter_v200(self, selected_tmp, selected_class, param):
        ...
        reduce_xy = self.instance.Tensor(self.dtype, (PRE_NMS_TOPN, ),
                                         name="reduce_xy", scope=tik.scope_ubuf)
        ...
        self.instance.vreduce(param["count_offset"], reduce_xy,
                              ...
                              1, 8, 1, 0, selected_class,
                              mask_mode="counter")
        repeats = (selected_class + 15) // 16
        self.instance.vconcat(selected_tmp, reduce_xy, repeats, 0)
        self.instance.vreduce(param["count_offset"], reduce_xy,
                              x1y1x2y2_ub[1, 0], pattern, 1,
                              1, 8, 1, 0, selected_class,
                              mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_xy, repeats, 1)
        self.instance.vreduce(param["count_offset"], reduce_xy,
                              x1y1x2y2_ub[2, 0], pattern, 1,
                              1, 8, 1, 0, selected_class,
                              mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_xy, repeats, 2)
        self.instance.vreduce(param["count_offset"], reduce_xy,
                              x1y1x2y2_ub[3, 0], pattern, 1,
                              1, 8, 1, 0, selected_class,
                              mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_xy, repeats, 3)
        self.instance.vreduce(param["count_offset"], reduce_xy, class_ub,
                              pattern, 1, 1, 8, 1, 0, selected_class,
                              mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_xy, repeats, 4)
    ...
    def move_result_to_gm(self, ret_ub, ret_label_ub, selected_count, batch):
        ...
        ret_label_ub: a tensor used to store label result
        selected_count: a scalar which store selected_count
