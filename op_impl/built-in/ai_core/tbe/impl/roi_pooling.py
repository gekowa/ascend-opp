...
def safe_check(dicts, kernel_name):
    ...
    if tik_name_check in ("Ascend310", "Ascend910", "Hi3796CV300ES", "Hi3796CV300CS"):
        ...
        op_utils.check_dtype(x_dtype, ["float16", "float32"], param_name="input_x")
        op_utils.check_dtype(rois_dtype, ["float16", "float32"], param_name="input_rois")
    ...
    if x_dtype != rois_dtype or x_dtype != y_dtype:
        ...
        raise RuntimeError(
            error_info, 
            "In op[roi_pooling], the dtype of tensor x, rois and y should be the same, but actually they are [%s]." % error_info['real_dtypes'])
    
    op_utils.check_shape(x_shape, min_rank=5, max_rank=5, param_name="input_x")
    op_utils.check_shape(rois_shape, min_rank=3, max_rank=3, param_name="input_rois")
    op_utils.check_shape(y_shape, min_rank=5, max_rank=5, param_name="output_y")
    ...
    if roi_max_num > 6000 or roi_max_num % 16 != 0:
        ...
        raise RuntimeError(
            error_info, 
            "In op[roi_pooling], the rois_shape[2] should be less than 6000 and can be divided by 16, but actually is [%s]." % error_info['real_rois_shape[2]'])


@op_utils.check_op_params(op_utils.REQUIRED_INPUT, op_utils.REQUIRED_INPUT, op_utils.OPTION_INPUT,
                          op_utils.REQUIRED_OUTPUT, op_utils.REQUIRED_ATTR_INT, op_utils.REQUIRED_ATTR_INT, 
                          op_utils.REQUIRED_ATTR_FLOAT, op_utils.REQUIRED_ATTR_FLOAT, op_utils.KERNEL_NAME)
