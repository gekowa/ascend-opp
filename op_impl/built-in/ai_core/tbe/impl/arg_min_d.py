...
def arg_min_d(x, y, dimension, kernel_name="arg_min_d"):
    ...
    kernel_name: str
        kernel name, default value is "arg_min_d"
...
class Argmin(ArgminBase):
    ...
    def do_not_last_fp16_default(self, segment, gm_in_offset, gm_out_offset):
        ...
        with self.tik_instance.for_range(1, self.axis_size,
            ...
            with self.tik_instance.for_range(0, repeat) as i:
                ...
                self.tik_instance.vcmpv_gt(ub_mask, ub_a[offset],
                                           ub_b[offset],
                                           1, 1, 1, 8, 8)
            ...
            self.tik_instance.vmin(self.full_mask, ub_a, ub_a,
                                   ub_b, repeat, 1,
                                   1, 1, 8, 8, 8)
    ...
    def do_not_last_fp16_align(self, segment, gm_in_offset, gm_out_offset):
        ...
        def _run_one_sigment(axis_idx, axis_len):
            ...
            with self.tik_instance.for_range(0, axis_len) as axis_i:
                ...
                with self.tik_instance.for_range(0, repeat) as i:
                    offset = i * self.data_each_vector
                    self.tik_instance.vcmpv_gt(ub_mask, ub_max[offset],
                ...
                self.tik_instance.vmin(self.full_mask, ub_max, ub_max,
    ...
    def argmin_last_axis(self):
        ...
        compute_fuction = self.compute_argmin_last_axis
        if self.dtype_x == "float16" and self.c_align_ubsize < MAX_SEGMENT_LEN:
            compute_fuction = self.compute_argmin_last_axis_copy_one_time

            if self.c_align_ubsize <= self.data_each_vector*2:
                compute_fuction = self.compute_argmin_last_axis_fp16_more_dims
        # calcu core policy
        core_number = self.product_core_num
        core_number_all = self.first_dim_size
    ...
    def compute_argmin_last_axis(self, n_i, core_segment, segment_core):
        ...
        def _run(segment_len, segment_index):
            with self.tik_instance.for_range(0, segment_len) as core_i:
                ...
                if self.dtype_x == "float16":
                    argmin_func = self.do_argmin_last_axis_fp16_default
                    self.result_out_scalar.set_as(SCALAR_MAX_FP16)
                else:
                    argmin_func = self.do_argmin_last_axis_fp32
                ...
                if loop_times != 0:
                    ...
                    with self.tik_instance.for_range(
                        ...
                        argmin_func(ub_buf_size, loop, offset)
                if align_flag:
                    argmin_func(over_size, loop_times, offset)
    ...
    def do_argmin_last_axis_fp16_default(self, ub_buf_size, loop, n_i):
        ...
        if repeat_times > 64:
            ...
            last_min_index.set_as(ub_result[third_min_index*64 + \
                                            second_min_index + 1])
            min_index = self.tik_instance.Scalar("uint16")
            min_index.set_as(third_min_index*64*64 + second_min_index*64 + \
                             last_min_index)
    ...
    def compute_argmin_last_axis_fp16_more_dims(self, n_i, core_segment,
                                                segment_core):
        """
        compute arg when do last axis
        ...
        def _run(segment_len, segment_index, axis_size_sigment):
            ...
            if self.c_align_ubsize >= self.data_each_vector:
                argmin_func = self.do_argmin_last_axis_fp16_more_vector
            else:
                argmin_func = self.do_argmin_last_axis_fp16_less_vector
            argmin_func(offset, segment_len)
    ...
    def do_argmin_last_axis_fp16_less_vector(self, offset, segment_len):
        """do arg in one segment fo float16
        """
        ...
        for align_idx, _ in enumerate(range(align_loop)):
            ...
            # get one axis vcmin
            self.tik_instance.vcmin(self.c_align_ubsize, copy_ub, copy_ub,
                                    repeat_vcmax, 1, 1,
                                    repeat_block)
    ...
    def do_argmin_last_axis_fp16_more_vector(self, offset, segment_len):
        """do arg in one segment fo float16
        """
        align_loop = get_align_num(self.c_align_ubsize,
                                   self.data_each_block)
    ...
    def compute_argmin_last_axis_copy_one_time(self, n_i, core_segment,
        ...
        def _run(segment_len, segment_index):
            with self.tik_instance.for_range(0, segment_len) as core_i:
                ...
                self.result_int32 = self.tik_instance.Scalar("int32")
                if self.dtype_x == "float16":
                    argmin_func = self.do_argmin_last_axis_fp16_copy_one_time
    ...
    def do_argmin_last_axis_fp16_copy_one_time(self, ub_buf_size, loop, n_i):
        """
        do arg in one segment fo float16
