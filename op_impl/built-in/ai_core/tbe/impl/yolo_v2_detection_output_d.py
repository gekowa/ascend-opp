...
def yolo_v2_detection_output_d(coord_data, obj_prob, classes_prob,
    ...
    Parameters
    ----------
    coord_data: dict, shape, dtype:fp16,fp32 format:only support NCHW
    obj_prob: dict, shape, dtype:fp16,fp32 format:only support NCHW
    classes_prob: dict, shape, dtype:fp16,fp32 format:only support NCHW
    img_info: dict, shape, dtype:fp16,fp32 format:only support NCHW
    windex: dict, shape, dtype:fp16,fp32 format:only support NCHW
    hindex: dict, shape, dtype:fp16,fp32 format:only support NCHW
    ...
    biases: the biases for correct box
    ...
    """
    batch = coord_data['shape'][0]
    height = windex['shape'][0]
    width = windex['shape'][1]
    dtype = windex['dtype']
...
class NmsComputer(ClsProbComputer):
    ...
    def __init__(self, input_dict):
        ...
        self.bbox =\
            self.instance.Tensor(self.dtype,
                                 (self.batch,
                                  self.max_box_number_per_batch * 6),
    ...
    def compute_detection_output(self):
        """
        ...
        self.instance.BuildCCE(kernel_name=self.kernel_name,
                               inputs=(self.coord_data, self.obj_prob,
                                       self.classes_prob, self.img_info,
                                       self.windex, self.hindex),
    ...
    def get_xyhw_by_index(self, xyhw_ub, param):
        ...
        index_offset_x.set_as(param["batch"] * 4 * self.obj_num)
        ...
        with self.instance.for_range(0, param["loop_cycle"]) as inner_cycle:
            ...
            # Fill the filtered coordinates in ub.
        ...
        if tbe_platform.cce_conf.get_soc_spec("SOC_VERSION") in ( \
            ...
            with self.instance.for_range(loop_start, param["count"]) as index:
                ...
                with self.instance.if_scope(offset < param["index_offset"]):
                    ...
                    # Filter the score value based on obj.
                    param["classes_ub_nms"][index].set_as(data)
        else:
            if self.obj_prob_v200.size // (8 * self.dsize) > self.max_ub_num:
    ...
    def class_filter(self, selected_tmp, proposals_ub, selected_class, param):
      ...
      """
        ...
        with self.instance.for_range(0, loop_cycle) as cmp_index:
            ...
            # Selection of a single proposal at a time
    ...
    def class_filter_v200(self, selected_tmp, selected_class, param):
        """
        filter by class threshold
        ...
        self.instance.vcmpvs_gt(pattern, class_ub, self.score_threshold, repeats, 1, 8)

        reduce_tmp = self.instance.Tensor(self.dtype, (PRE_NMS_TOPN, ),
                                          name="reduce_tmp", scope=tik.scope_ubuf)
        self.instance.vec_dup(self.mask, reduce_tmp, 0.0,
                              PRE_NMS_TOPN * self.dsize // 256, 8)

        self.instance.vreduce(param["count_offset"], reduce_tmp, class_ub, pattern,
                              1, 1, 8, 1,
                              0, selected_class, mask_mode="counter")

        repeats = (selected_class + 15) // 16
        self.instance.vconcat(selected_tmp, reduce_tmp, repeats, 4)

        self.instance.vreduce(param["count_offset"], reduce_tmp, x1y1x2y2_ub[0, 0], pattern,
                              1, 1, 8, 1,
                              0, selected_class, mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_tmp, repeats, 0)

        self.instance.vreduce(param["count_offset"], reduce_tmp, x1y1x2y2_ub[1, 0], pattern,
                              1, 1, 8, 1,
                              0, selected_class, mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_tmp, repeats, 1)

        self.instance.vreduce(param["count_offset"], reduce_tmp, x1y1x2y2_ub[2, 0], pattern,
                              1, 1, 8, 1,
                              0, selected_class, mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_tmp, repeats, 2)

        self.instance.vreduce(param["count_offset"], reduce_tmp, x1y1x2y2_ub[3, 0], pattern,
                              1, 1, 8, 1,
                              0, selected_class, mask_mode="counter")
        self.instance.vconcat(selected_tmp, reduce_tmp, repeats, 3)
    ...
    def nms_filter(self, proposals_selected, selected_tmp, selected_count,
        ...
        with self.instance.for_range(0, cycle_roi) as count_cycle:
            ...
            self.instance.vec_muls(self.mask, tempjoin_ub[0],
                                   tempjoin_ub[0],
            ...
            with self.instance.for_range(0, 16) as j:
                ...
                with self.instance.if_scope(
                        tik.all(tmp + 1 <= selected_class,
                                supvec_ub[tmp] == 0,
                                selected_count < self.max_box_number_per_batch,
                                iou_count < self.post_nms_topn)):
                    index_tmp = tmp * 8
                    if self.dtype == constant.DATA_TYPE_FP32:
    ...
    def nms_of_each_class(self, param, selected_count, proposals_selected):
        ...
        self.instance.vec_adds(self.mask, param["class_ret_ub"],
                               param["class_ret_ub"], 1.0, 1,
                               constant.REPEAT_STRIDE_EIGHT,
                               constant.REPEAT_STRIDE_EIGHT)
        # iou_count: remaining number after iou filtering.
    ...
    def process_each_class(self, proposals_ub, mask, param):
        ...
        with self.instance.if_scope(param["count"] > 0):
            with self.instance.for_range(0, self.classes) as class_cycle:
                with self.instance.if_scope(selected_count <
                                            self.max_box_number_per_batch):
    ...
    def multi_class_boxes(self, ret_ub, image_ub, selected_count):
        ...
        if tbe_platform.cce_conf.get_soc_spec("SOC_VERSION") in ( \
            ...
            self.instance.vec_max(self.mask, ret_ub[1, 0], ret_ub[1, 0],
                                  ...
                                  constant.REPEAT_STRIDE_EIGHT,
                                  constant.REPEAT_STRIDE_EIGHT)
            self.instance.vec_dup(self.mask, threshold[0], image_w,
                                  repeats, constant.REPEAT_STRIDE_EIGHT)
            self.instance.vec_adds(self.mask, threshold, threshold, -1,
                                   repeats, constant.REPEAT_STRIDE_EIGHT,
                                   constant.REPEAT_STRIDE_EIGHT)

            self.instance.vec_min(self.mask, ret_ub[2, 0],
                                  ret_ub[2, 0], threshold[0], repeats,
                                  constant.REPEAT_STRIDE_EIGHT,
                                  constant.REPEAT_STRIDE_EIGHT,
                                  constant.REPEAT_STRIDE_EIGHT)
            ...
            img_wh = self.instance.Tensor(self.dtype, (4,),
                                          scope=tik.scope_ubuf,
                                          name="threshold")
            self.instance.vec_adds(4, img_wh, image_ub, -1, 1, 8, 8)
            self.instance.vmaxs(self.mask, ret_ub[0, 0], ret_ub[0, 0], 0,
                                repeats, constant.STRIDE_ONE,
                                constant.STRIDE_ONE,
                                constant.REPEAT_STRIDE_EIGHT,
                                constant.REPEAT_STRIDE_EIGHT)
            self.instance.vmaxs(self.mask, ret_ub[1, 0], ret_ub[1, 0], 0,
                                repeats, constant.STRIDE_ONE,
                                constant.STRIDE_ONE,
                                constant.REPEAT_STRIDE_EIGHT,
                                constant.REPEAT_STRIDE_EIGHT)

            thres = self.instance.Scalar(dtype=self.dtype)
            thres.set_as(img_wh[3]) # w

            self.instance.vmins(self.mask, ret_ub[2, 0], ret_ub[2, 0],
                                thres, repeats, constant.STRIDE_ONE,
                                constant.STRIDE_ONE,
                                constant.REPEAT_STRIDE_EIGHT,
                                constant.REPEAT_STRIDE_EIGHT)

            thres.set_as(img_wh[2]) # h
            self.instance.vmins(self.mask, ret_ub[3, 0], ret_ub[3, 0],
                                thres, repeats, constant.STRIDE_ONE,
                                constant.STRIDE_ONE,
                                constant.REPEAT_STRIDE_EIGHT,
                                constant.REPEAT_STRIDE_EIGHT)
    ...
    def get_xyhw(self, obj_ub, mask, xyhw_ub, param):
        ...
        self.data_move_xyhw(coords_ub_xyhw, param)
        scalar = self.instance.Scalar("uint32")
        mask_tmp = self.instance.Scalar()
        mask_tmp.set_as(param["obj_total"] // self.dsize)
        with self.instance.if_scope(
                param["count"] * self.dsize % constant.BLOCK_SIZE \
                == 0):
    ...
    def get_nonaligned_xyhw(self, mask, coords_ub_xyhw, xyhw_ub, param):
        ...
        self.instance.vreduce(param["mask_tmp"], reduce_xyhw,
                              ...
                              8, 1,
                              0, param["scalar"], mask_mode="counter")
        with self.instance.if_scope(
        ...
        self.instance.vreduce(param["mask_tmp"], reduce_xyhw,
                              ...
                              mask[param["mask_offset"]], param["repeats"], 1,
                              8, 1,
                              0, scalar_tmp, mask_mode="counter")
        with self.instance.for_range(0, param["scalar"]) as index:
