...
from impl.max_pool_grad_with_argmax_cut_one_h_v2 import MaxpoolGradCustom
...
class MaxpoolGard(MaxpoolGradCustom):
    ...
    def tik_instance_shape_large_shape(self, kernel_name):
        """
        function for max_pool_grad_with_pool calc for large shape,
        ksize or windows special
        """
        ...
        data_mask = self.tik_instance.Tensor("uint16", (
            batch * channel1 * windowh * windoww * \
            mask_one_window,),
                                             name="data_mask",
                                             scope=tik.scope_gm)
        ...
        with self.tik_instance.for_range(0, real_block,
                                         block_num=real_block) as block_id:
            ...
            with self.tik_instance.for_range(0, real_cycle) as cycle_id:
                ...
                self.clean_gm(dtype, num_one_block, block_num * num_one_block,
                              data_output)
                with self.tik_instance.for_range(0, nc1,
                                                 thread_num=1) as loopc1:
                    ...
                    data_mask_ub = self.tik_instance.Tensor("uint16", (
                        h_input * col2img_dyw * windowh * windoww,),
                                                        name="data_mask_ub",
                                                        scope=tik.scope_ubuf
                                                            )
                    ...
                    with self.tik_instance.for_range(0, dyh,
                                                     thread_num=1) as looph:
                        if hoverlap == 0:
                        ...
                        with self.tik_instance.for_range(0, dyw) as loopw:
                            ...
                            self.clean_fp32_multi_repeat(
                                data_vmul_ub_col2img_fp32, dtype_size * 2)
                            # dy copy gm to ub
                            ...
                            if h_ratio != 0:
                                self.tik_instance.data_move(data_max_ub,
                                    data_input[
                                    block_num * nc1 * dyh * dyw *
                                    channel + loopc1 * dyh * dyw * channel +
                                    (
                                            new_looph + 1) * dyw * channel +
                                    channel * new_loopw],
                                    constant.SID, 1,
                                    constant.DEFAULT_BURST_LEN + w_ratio,
                                    0, 0)
                            ...
                            self.tik_instance.data_move(data_mask_ub,
                                                        data_mask[
                                block_num * nc1 * mask_one_window *
                                windoww * windowh + loopc1 *
                                mask_one_window * windoww * windowh +
                                new_looph * dyw + new_loopw],
                                                    constant.SID,
                                                    windowh * windoww,
                                                    constant.DEFAULT_BURST_LEN,
                                                    mask_stride,
                            ...
                            if hoverlap != 0:
                                self.tik_instance.data_move(data_mask_ub[
                                            col2img_dyw * windowh * windoww],
                                                            data_mask[
                                block_num * nc1 * mask_one_window *
                                windoww * windowh + loopc1 * mask_one_window *
                                windoww * windowh + (
                                        new_looph + 1) * dyw +
                                loopw],
                            constant.SID,
                            ...
                            with self.tik_instance.for_range(0,
                                                windowh * windoww) as mask_id:
                                cmpmask = \
                                    self.tik_instance.mov_tensor_to_cmpmask(
                                ...
                                if hoverlap != 0:
                                    cmpmask = \
                                    self.tik_instance.mov_tensor_to_cmpmask( \
        data_mask_ub[col2img_dyw * mask_id + col2img_dyw * windowh * windoww])
                            ...
                            self.calc_dma_param(block_num, burst_len, channel,
                                                col2img_w,
                                                dst_address, dst_stride, dxh,
                                                dxw, dyh,
                                                dyw, hoverlap, loopc1, looph,
                                                loopw,
                                                nburst, num_one_block,
                                                num_one_c0, pad_left,
                                                pad_top, src_address,
                                                src_stride, strideh,
                                                stridew, windowh, windoww,
                                                woverlap,
                                                pad_bottom, pad_right)

                            # move ub to gm
                            self.tik_instance.data_move(
                                data_output[dst_address],
                                data_vmul_ub_col2img_fp16[src_address],
                                constant.SID, nburst,
                                burst_len, src_stride, dst_stride)
        self.tik_instance.BuildCCE(kernel_name=kernel_name,
                                   inputs=(
                                       data_input_origin, data_input,
                                       data_mask),
                                   outputs=(data_output), enable_l2=False)
        return self.tik_instance

    # pylint: disable=locally-disabled,too-many-statements
    def calc_dma_param(self, block_num, burst_len, channel, col2img_w,
                       dst_address, dst_stride, dxh,
                       dxw, dyh, dyw, hoverlap, loopc1, looph, loopw, nburst,
                       num_one_block,
        ...
        if hoverlap == 0:
            ...
            if woverlap == 0:
                ...
                with self.tik_instance.else_scope():
                    dst_address.set_as(
                        dst_address + (loopw * stridew - pad_left) * channel)
            ...
            with self.tik_instance.else_scope():
                ...
                dst_address.set_as(
                    dst_address + (looph * strideh - pad_top) * dxw * channel)
                with self.tik_instance.if_scope(looph == dyh - 1):
    ...
    def tik_instance_function(self, kernel_name):
        ...
        if windowh > 2 * strideh or windoww > 2 * stridew:
            if col2img_w * col2img_one_h * channel * dtype_size \
                    > self.ub_limit:
                raise RuntimeError(
                    "The shape or ksize or stride is too large, please check!")
