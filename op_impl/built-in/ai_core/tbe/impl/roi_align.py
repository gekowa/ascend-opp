...
class RoiAlign():
    ...
    def roialign_compute_each_core(self, block_num, block_offset, block_left):
        ...
        if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
            ...
            roi_gridh_fp32 = self.tik_inst.Tensor("float16", (128, ), \
                                                  scope=tik.scope_ubuf, name="roi_gridh_fp32")
            roi_gridw_fp32 = self.tik_inst.Tensor("float16", (128, ), \
                                                  scope=tik.scope_ubuf, name="roi_gridw_fp32")
            x_start_fp32_ub = self.tik_inst.Tensor("float16", (ROINUM,), \
                                                   name="x_start_fp32_ub", scope=tik.scope_ubuf)
            y_start_fp32_ub = self.tik_inst.Tensor("float16", (ROINUM,), \
                                                   name="y_start_fp32_ub", scope=tik.scope_ubuf)
        ...
        with self.tik_inst.new_stmt_scope():
            ...
            if cce_product == "Hi3796CV300CS":
                index_arr_int16 = self.tik_inst.Tensor("int16", (ROINUM, ), \
                                                   name="index_arr_int16", scope=tik.scope_ubuf)
                self.tik_inst.vcbd(64, index_arr_int16, index_arr_int32, 2, 1, 1, 4, 8)
                self.tik_inst.vec_conv(128, '', index_arr, index_arr_int16, 1, 8, 8)
            else:
    ...
    def roi_align_perf_scale(self, rois, xstart, roisnum, sample_ratio,
        ...
        with self.tik_inst.new_stmt_scope():
            ...
            if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
                ...
                x_end_fp32_ub = self.tik_inst.Tensor("float16", (ROINUM,), \
                                                     name="x_end_fp32_ub", scope=tik.scope_ubuf)
            ...
            if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
                ...
                dtype_num = 2
                self.tik_inst.vec_muls(128, x_start_fp32_ub, roi_buf1_ub[0, 0], \
                                       spatialScale_fp32, 1, 8, 8)
                self.tik_inst.vec_muls(128, y_start_fp32_ub, roi_buf1_ub[1, 0], \
                                       spatialScale_fp32, 1, 8, 8)
                #add roi_end_mode
                if roi_end_mode > 0:
                    self.tik_inst.vec_adds(128, x_end_fp32_ub, roi_buf1_ub[2, 0], \
                                           roi_end_mode, 1, 8, 8)
                    self.tik_inst.vec_adds(128, y_end_fp32_ub, roi_buf1_ub[3, 0], \
                                           roi_end_mode, 1, 8, 8)

            self.tik_inst.vec_muls(64 * dtype_num, x_end_fp32_ub, x_end_fp32_ub, \
                                   spatialScale_fp32, 2 // dtype_num, 8, 8)
            ...
            if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
                ...
                self.tik_inst.vec_sub(128, roi_h_ub, y_end_fp32_ub, \
                                      y_start_fp32_ub, 1, 8, 8, 8)  # h == yEndFp32
                self.tik_inst.vec_sub(128, roi_w_ub, x_end_fp32_ub, \
                                      x_start_fp32_ub, 1, 8, 8, 8)  # w == xEndFp32
            ...
            if (sample_ratio > 0):
                if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
                    ...
                    self.tik_inst.vec_muls(128, roi_gridw_fp32, roi_w_ub, \
                                           scaleGW_fp32, 1, 8, 8)
                    self.tik_inst.vec_muls(128, roi_gridh_fp32, roi_h_ub, \
                                           scaleGH_fp32, 1, 8, 8)
            else:
                ...
                if cce_product == "Hi3796CV300CS":
                    n_grid_w_int16 = self.tik_inst.Tensor("int16", (ROINUM, ), \
                                                       name="n_grid_w_int16", scope=tik.scope_ubuf)
                    n_grid_h_int16 = self.tik_inst.Tensor("int16", (ROINUM, ), \
                                                       name="n_grid_h_int16", scope=tik.scope_ubuf)
                    ...
                    self.tik_inst.vcbd(64, n_grid_h_int16, n_grid_h_int_ub, 2, 1, 1, 4, 8)
                    self.tik_inst.vec_conv(128, '', n_grid_w_ub, n_grid_h_int16, 1, 8, 8)
                ...
                if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
                    ...
                    self.tik_inst.vec_mul(128, roi_gridw_fp32, roi_bin_w_ub, \
                                          n_grid_rec_w_ub, 1, 8, 8, 8)
    ...
    def roialign_perf_gen_grid(self,
        ...
        if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
            ...
            delta_w = tik_instance.Scalar("float16", name="delta_w")
            delta_h = tik_instance.Scalar("float16", name="delta_h")
            w0 = tik_instance.Scalar("float16", name="w0")
            h0 = tik_instance.Scalar("float16", name="h0")
        ...
        if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
            ...
            tik_instance.vec_muls(MASK_FP16, xpos_fp32, index_arr, delta_w, \
                                  1, 8, 8)
        ...
        if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
            ...
            tik_instance.vec_conv(MASK_FP32, 'floor', xlow_int, xpos_fp32, \
                                  2, 8, 4)   # fp16  -> int32
        ...
        if cce_product == "Hi3796CV300CS":
            xlow_int16 = self.tik_inst.Tensor("int16", (ROINUM, ), \
                                               name="xlow_int16", scope=tik.scope_ubuf)
            ylow_int16 = self.tik_inst.Tensor("int16", (ROINUM, ), \
                                               name="ylow_int16", scope=tik.scope_ubuf)
        ...
        if cce_product not in ("Hi3796CV300ES", "Hi3796CV300CS"):
            ...
            tik_instance.vec_sub(MASK_FP16, lx, xpos_fp32, xlow, \
                                 1, 8, 8, 8)
            tik_instance.vec_sub(MASK_FP16, ly, ypos_fp32, ylow, \
                                 1, 8, 8, 8)
            tik_instance.vec_sub(MASK_FP16, hx, xhigh, xpos_fp32, \
                                 1, 8, 8, 8)
            tik_instance.vec_sub(MASK_FP16, hy, yhigh, ypos_fp32, \
                                 1, 8, 8, 8)
...
def roi_align_cce(feature_map_dict, rois_dict, roisn_dict, output_dict, \
    ...
    input_c0 = input_shape[4]
    pooled_h = output_h
    pooled_w = output_w
    max_sample_num = COM_LIMIT_SAMPLE_NUM
    if cce_product == "Hi3796CV300ES":
...
def get_roi_align_perf_scale_for_zero_V200(tik_instance, roi_fp32_fm_index, proposals_ub_x0,
    ...
    grid_w_int16 = tik_instance.Tensor(
        "int16", [proposal_num_128], name="grid_w_int16", scope=tik.scope_ubuf)
    grid_h_int16 = tik_instance.Tensor(
        "int16", [proposal_num_128], name="grid_h_int16", scope=tik.scope_ubuf)
    ...
    if sample_num <= 0:
        ...
        if suppot_vconv is False and dtype == "float32":
            if cce_product == "Hi3796CV300CS":
                ...
                tik_instance.vec_conv(128, '', grid_h_fp16, grid_h_int16, 1, 8, 8)
            else:
            ...
            if dtype == "float32":
                ...
                if cce_product == "Hi3796CV300CS":
                    ...
                    tik_instance.vcbd(64, grid_h_int16, grid_h_int32, 2, 1, 1, 4, 8)
                    tik_instance.vec_conv(128, '', grid_h_fp32, grid_h_int16, 1, 8, 8)
                else:
                    tik_instance.vec_conv(64, "", grid_w_fp32, grid_w_int32, 2,
                                          8 // dtype_num, 8, 1.0)
                    tik_instance.vec_conv(64, "", grid_h_fp32, grid_h_int32, 2,
...
def get_grid_weight_per_roi(tik_instance, roi_bin_h_fp32_value,
    ...
    if vconv_suppot is False or dtype == "float16":
        ...
        if cce_product == "Hi3796CV300CS":
            ...
            "int16", [1], name="tmp_int16", scope=tik.scope_ubuf)
            tik_instance.vcbd(1, tmp_int16, tmp_int32, 1, 1, 1, 4, 8)
            tik_instance.vec_conv(1, '', tmp_float16, tmp_int16, 1, 8, 8)
        else:
        ...
        if dtype == "float32":
            ...
            if cce_product == "Hi3796CV300CS":
                tmp_int16 = tik_instance.Tensor(
                    "int16", [1], name="tmp_int16", scope=tik.scope_ubuf)
                tik_instance.vcbd(1, tmp_int16, tmp_int32, 1, 1, 1, 4, 8)
                tik_instance.vec_conv(1, '', const_value_fp32, tmp_int16, 1, 8, 8)
            else:
    ...
    if vconvf_suppot is False and dtype == "float32":
        if cce_product == "Hi3796CV300CS":
            roi_bin_ph_gh_ly_int16 = tik_instance.Tensor(
                "int16", [1], name="roi_bin_ph_gh_ly_int16", scope=tik.scope_ubuf)
            tik_instance.vcbd(1, roi_bin_ph_gh_ly_int16, roi_bin_ph_gh_ly_int32, 1, 1, 1, 4, 8)
            tik_instance.vec_conv(1, '', tmp_float16, roi_bin_ph_gh_ly_int16, 1, 8, 8)
        else:
        ...
        if dtype == "float32":
            ...
            if cce_product == "Hi3796CV300CS":
                ...
                tik_instance.vec_conv(1, '', tmp_bin_h_fp32[0], roi_bin_ph_gh_ly_int16, 1, 8, 8)
            else:
                tik_instance.vec_conv(1, "", tmp_bin_h_fp32[0],
                                      roi_bin_ph_gh_ly_int32[0],
                                      1, 8 // dtype_num, 8, 1.0)
...
def prepare_vbi_xn(tik_instance, c1_block_num):
    '''
    :param tik_instance:
    ...
    for current_c1 in range(c1_block_num):
        c1_offset = tik_instance.Scalar(dtype="float16", \
                            init_value=1024 * current_c1)
        c1_offset_tensor = tik_instance.Tensor( \
            "int32", [1], name="c1_offset_tensor", scope=tik.scope_ubuf)
        c1_offset_tensor[0].set_as(c1_offset)
        for horizontal_repeat_time in range(4):
            offset_num = tik_instance.Scalar(dtype="float16", \
                                 init_value=32 * horizontal_repeat_time)
            start_offset = tik_instance.Tensor( \
                "int32", [1], name="start_offset", scope=tik.scope_ubuf)
            ...
            for c1_time in range(8):
                ...
                c1_time_tensor = tik_instance.Tensor( \
                    "int32", [1], name="c1_time_tensor", scope=tik.scope_ubuf)
...
def prepare_vbi_xm(tik_instance, h_y, l_y, h_x, l_x, c1_block_num):
    '''
    :param tik_instance:
    ...
    hy_tensor = tik_instance.Tensor( \
        "float16", [1], name="hy_tensor", scope=tik.scope_ubuf)
    hy_tensor[0].set_as(h_y)
    ly_tensor = tik_instance.Tensor( \
        "float16", [1], name="ly_tensor", scope=tik.scope_ubuf)
    ly_tensor[0].set_as(l_y)
    hx_tensor = tik_instance.Tensor( \
        "float16", [1], name="hx_tensor", scope=tik.scope_ubuf)
    hx_tensor[0].set_as(h_x)
    lx_tensor = tik_instance.Tensor( \
        "float16", [1], name="lx_tensor", scope=tik.scope_ubuf)
    lx_tensor[0].set_as(l_x)
        ...
        with tik_instance.if_scope(verify == 0):
            ...
            with tik_instance.for_range(0, pw_int * sample_num_w) as grid_num_w:
                ...
                with tik_instance.if_scope(verify == 0):
                    ...
                    if (cce_product in ("Ascend610", "Ascend710", "Hi3796CV300CS")) \
                       and (dtype == "float16") and (fm_c1 >= 8):
                        c1_block_num = (fm_c1 + 7) // 8
                        vbi_weights = prepare_vbi_xm(tik_instance, h_y, l_y, h_x, \
                                                     l_x, c1_block_num)
                        vbi_addr = prepare_vbi_xn(tik_instance, c1_block_num)
    ...
    if vconv_f322s32f_suppot is False and dtype == "float32":
        if cce_product == "Hi3796CV300CS":
            x_lo_int16 = tik_instance.Tensor("int16", (ROINUM, ), \
                            name="x_lo_int16", scope=tik.scope_ubuf)
    ...
    if vconv_f322s32f_suppot is False and dtype == "float32":
        if cce_product == "Hi3796CV300CS":
            y_lo_int16 = tik_instance.Tensor("int16", (ROINUM, ), \
                            name="y_lo_int16", scope=tik.scope_ubuf)
    ...
    with tik_instance.for_range(
        ...
        with tik_instance.if_scope(rois_valid != 0):
            with tik_instance.for_range(
                ...
                if (cce_product in ("Ascend610", "Ascend710")) and (dtype == "float16") \
                   and (rois_shape[1] == 5):
                    rois_ub_n5 = tik_instance.Tensor(
                        dtype, [128 * 5], name="rois_ub_n5",
                        scope=tik.scope_ubuf)
                    tik_instance.data_move(rois_ub_n5[0],
                    ...
                    src1_ub_fp16 = tik_instance.Tensor("float16", (128,), \
                                       name="src1_ub_fp16", scope=tik.scope_ubuf)
                    src1_ub_uint16 = tik_instance.Tensor("uint16", (8,), \
                                       name="src1_ub_uint16", scope=tik.scope_ubuf)
                    ...
                    tik_instance.vec_add([0x0000000000001084, 0x2108421084210842], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, proposals_ub_x0[0, 0], rois_ub_n5, \
                                         src1_ub_uint16, 7, 1, 5, 0, 0, None, "normal")
                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x1084210842108421, 0x0842000000000000], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, proposals_ub_x0[112, 0], rois_ub_n5[128 * 4], \
                                         src1_ub_uint16, 1, 1, 8, 0, 0, None, "normal")

                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x0000000000002108, 0x4210842108421084], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, proposals_ub_y0[0, 0], rois_ub_n5, \
                                         src1_ub_uint16, 7, 1, 5, 0, 0, None, "normal")
                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x2108421084210842, 0x1084000000000000], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, proposals_ub_y0[112, 0], rois_ub_n5[128 * 4], \
                                         src1_ub_uint16, 1, 1, 8, 0, 0, None, "normal")

                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x0000000000004210, 0x8421084210842108], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, proposals_ub_x1[0, 0], rois_ub_n5, \
                                         src1_ub_uint16, 7, 1, 5, 0, 0, None, "normal")
                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x4210842108421084, 0x2108000000000000], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, proposals_ub_x1[112, 0], rois_ub_n5[128 * 4], \
                                         src1_ub_uint16, 1, 1, 8, 0, 0, None, "normal")

                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x0000000000008421, 0x0842108421084210], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(80, proposals_ub_y1[0, 0], rois_ub_n5, \
                                         src1_ub_uint16, 7, 1, 5, 0, 0, None, "normal")
                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x8421084210842108, 0x4210000000000000], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, proposals_ub_y1[112, 0], rois_ub_n5[128 * 4], \
                                         src1_ub_uint16, 1, 1, 8, 0, 0, None, "normal")

                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    roi_fm_index = tik_instance.Tensor(
                        dtype, [128], name="roi_fm_index", scope=tik.scope_ubuf)
                    tik_instance.vec_add([0x0000000000000842, 0x1084210842108421], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(80, roi_fm_index[0], rois_ub_n5, \
                                         src1_ub_uint16, 7, 1, 5, 0, 0, None, "normal")
                    tik_instance.vector_dup(128, src1_ub_fp16, 0, 1, 1, 8)
                    tik_instance.vec_add([0x0842108421084210, 0x8421000000000000], \
                                         src1_ub_fp16, src1_ub_fp16, one_ub, 1, 8, 8, 8)
                    tik_instance.vec_cmpv_eq(src1_ub_uint16, src1_ub_fp16, one_ub, 1, 8, 8)
                    tik_instance.vreduce(128, roi_fm_index[112], rois_ub_n5[128 * 4], \
                                         src1_ub_uint16, 1, 1, 8, 0, 0, None, "normal")
                    ...
                    grid_w_int32, grid_h_int32, grid_w_fp32, \
                    grid_h_fp32, roi_int32_fm_index = \
                        get_roi_align_perf_scale_for_zero_V200(tik_instance, roi_fm_index,
                                                          proposals_ub_x0,
                                                          proposals_ub_y0,
                                                          proposals_ub_x1,
                                                          proposals_ub_y1,
...
@check_op_params(REQUIRED_INPUT, REQUIRED_INPUT, OPTION_INPUT,
                 REQUIRED_OUTPUT, REQUIRED_ATTR_FLOAT, REQUIRED_ATTR_INT,
                 REQUIRED_ATTR_INT, OPTION_ATTR_INT, OPTION_ATTR_INT, KERNEL_NAME)
def roi_align(feature_map_dict,
    ...
    if ((cce_product in ("Ascend610", "Ascend710")) \
        ...
        return roi_align_vbi.roi_align_vbi(feature_map_dict, \
         ...
         (cce_product in ("Ascend310", "Ascend910", "Hi3796CV300ES", "Ascend610", "Ascend710")):
