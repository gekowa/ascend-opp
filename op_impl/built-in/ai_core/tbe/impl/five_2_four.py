...
def _move_ci_align_nhwc_fp16_fencore(args):
    """
    function of moving data for ci align nhwc float16 fencore scene

    """
    tvm_ib, data, dst, data_ub, data_res, cp_align_len, col_len_shape,\
    n_index, col_before, col_now = args
    ...
    with tvm_ib.for_range(0, c_1, name="num_c1") as num_c1:
        with tvm_ib.if_scope(vadds_zu > 0):
            ...
            _func_vadds_for_move(args)
        with tvm_ib.if_scope(vadds_mod > 0):
            ub_offset = num_c1 * col_now * c_0 + vadds_zu * 8 * c_0
            res_offset = num_c1 * c_0 + vadds_zu * 8 * c_i
            repeat = 1
            srcm0 = 1
            dstm0 = c_1
    ...
    dst_offset = n_index * h_i * w_i * c_i + col_before * c_i
    burst_len_dst = col_now * c_1
    tvm_ib.emit(tvm.call_extern(dst.dtype, "copy_ubuf_to_gm",
                                dst.access_ptr('w',
                                               offset=dst_offset),
                                data_res.access_ptr("r",
...
def _func_ci_align_nhwc_fp16_nomod(args):
    """
    function of ci align nhwc float16 nomod scene

    """
    tvm_ib, data, dst, data_ub, data_res, device_core_num, block_index, \
    cp_align_len, col_len_shape, col_len_ub, ub_mod, fen_n, num_g = args
    ...
    col_now = col_len_ub
    col_before = col_len_ub * fen_index
    args = tvm_ib, data, dst, data_ub, data_res, cp_align_len, \
           col_len_shape, n_index, col_before, col_now
    _move_ci_align_nhwc_fp16_fencore(args)


def _func_ci_align_nhwc_fp16_mod(args):
    """
    function of ci align nhwc float16 mod scene
...
def _ci_align_nhwc_fp16_fencore(dst, data):
    """
    function of making ir node builder for ci align nhwc float16 fencore scene
    ...
    with tvm_ib.if_scope(ub_mod > 0):
        ...
        with tvm_ib.for_range(0, group_index, name="num_g") as num_g:
            args = tvm_ib, data, dst, data_ub, data_res, device_core_num,\
                   block_index, cp_align_len, col_len_shape, col_len_ub,\
                   ub_mod, fen_n, num_g
            _func_ci_align_nhwc_fp16_mod(args)
        with tvm_ib.if_scope(group_mod > 0):
            with tvm_ib.if_scope(block_index < group_mod):
                args = tvm_ib, data, dst, data_ub, data_res, device_core_num,\
                       block_index, cp_align_len, col_len_shape, col_len_ub,\
                       ub_mod, fen_n, group_index
                _func_ci_align_nhwc_fp16_mod(args)
    ...
    with tvm_ib.else_scope():
        ...
        with tvm_ib.for_range(0, group_index, name="num_g") as num_g:
            args = tvm_ib, data, dst, data_ub, data_res, device_core_num,\
                   block_index, cp_align_len, col_len_shape, col_len_ub,\
                   ub_mod, fen_n, num_g
            _func_ci_align_nhwc_fp16_nomod(args)
        with tvm_ib.if_scope(group_mod > 0):
            with tvm_ib.if_scope(block_index < group_mod):
                args = tvm_ib, data, dst, data_ub, data_res, device_core_num,\
                       block_index, cp_align_len, col_len_shape, col_len_ub,\
...
def _func_n_16_7_7_nchw_fp16(args):
    """
    function of n,16,7,7 nchw fp16 scene
...
def _n_16_7_7_nchw_fp16(dst, data):
    """
    function of making ir node builder for n,16,7,7 nchw fp16 scene
...
def _check_ci_align_nhwc_fencore(src_shape, dst_shape, dst_format, dtype):
    """
    check whether to use ci align nhwc fencore branch
    ...
    if col_len_shape > col_len_ub and n_i < AICORE_NUM:
        return True
...
def _check_n_16_7_7_nchw_fp16(dst_shape, dst_format, dtype):
    """
    check whether n,16,7,7 nchw fp16 scene
    ...
    sub_shape = dst_shape[1:]
    if list(sub_shape) != [16, 7, 7]:
        return False
    ...
    subshape_ele = functools_reduce(lambda x, y: x * y, sub_shape[:])
    in_ele = subshape_ele * 16
