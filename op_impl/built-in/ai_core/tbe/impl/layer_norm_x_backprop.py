...
def _check_shape_gamma(shape_x, shape_gamma):
    ...
    if len(shape_gamma) > len(shape_x):
        raise RuntimeError("length of shape_gamma can not be longer "
                           "than shape_x")
...
@fusion_manager.register("layer_norm_x_backprop")
...
def update_shape_nz(shape_x, shape_var, shape_gamma):
    ...
    param_nz = {"shape_x_nz": shape_x_nz, "shape_var_nz": shape_var_nz,
                ...
                "param_nz_axis": param_nz_axis, "mean_nz_num": mean_nz_num}
...
def _get_data_nz(param_nz, dtype):
    ...
    """
    data_dy = tvm.placeholder(param_nz.get("shape_x_nz"),
                              name="data_dy", dtype=dtype)
    data_x = tvm.placeholder(param_nz.get("shape_x_nz"),
                             name="data_x", dtype=dtype)
    data_variance = tvm.placeholder(param_nz.get("shape_var_nz"),
                                    name="data_variance", dtype=dtype)
    data_mean = tvm.placeholder(param_nz.get("shape_var_nz"),
                                name="data_mean", dtype=dtype)
    data_gamma = tvm.placeholder(param_nz.get("shape_gamma_nz"),
                                 name="data_gamma", dtype=dtype)
...
def _get_pd_xl_nz(data, param_nz):
    ...
    """
    data_gamma_cast = te.lang.cce.broadcast(data.get("data_gamma"),
...
def _get_pd_var_nz(data, param_nz, pd_xl, cast_dtype):
    ...
    pdvar_mul1 = te.lang.cce.vmul(pd_xl, sub_x_mean)
    pdvar_sum = te.lang.cce.sum(pdvar_mul1, param_nz.get("reduce_nz_axis"),
                                keepdims=True)
    pdvar_mul3 = te.lang.cce.vmul(pdvar_sum, pd_var_1)
    pd_var = te.lang.cce.vmuls(pdvar_mul3, tvm.const(-0.5, dtype=cast_dtype))
...
def _get_pd_mean_nz(param_nz, pd_xl, pd_var, var_elta_2, sub_x_mean,
                    cast_dtype):
    """
    ...
    """
    pdmean1_sum = te.lang.cce.sum(pd_xl, param_nz.get("reduce_nz_axis"),
                                  keepdims=True)
    pdmean1_mul = te.lang.cce.vmul(pdmean1_sum, var_elta_2)
    pd_mean_1 = te.lang.cce.vmuls(pdmean1_mul,
                                  tvm.const(-1.0, dtype=cast_dtype))
...
def _get_pd_x_nz(data, param_nz, dtype, cast_dtype):
    ...
    """
    pd_x_1, pd_x_2, pd_x_3 = _get_pd_x_front_nz(data, param_nz, cast_dtype)
...
def _get_res_nz(data, param_nz, dtype, cast_dtype):
    ...
    """
    pd_x = _get_pd_x_nz(data, param_nz, dtype, cast_dtype)

    return pd_x


def _get_pds_nz(data_dy, data_x, data_variance, data_mean,
                data_gamma, param_nz):
    """
    get params and data, compute pd_x, pd_gamma, pd_beta.
    ...
    pd_x = _get_res_nz(data, param_nz, dtype, cast_dtype)

    return pd_x


def layer_norm_x_back_nz_compute(data_dy, data_x, data_variance,
                                 data_mean, data_gamma, param_nz):
    """
    DSL description of the layernorm_grad operator's mathematical
    calculation process
    ...
    res_tuple: tuple
        (pd_x, pd_gamma, pd_beta)
    """
    pd_x = _get_pds_nz(data_dy, data_x, data_variance,
                       data_mean, data_gamma, param_nz)
...
def layer_norm_x_backprop(input_dy, input_x, input_variance, input_mean,
    ...
    if format_dy.upper() == "FRACTAL_NZ":
        ...
        pd_x = layer_norm_x_back_nz_compute(data_dy, data_x, data_variance,
                                            data_mean, data_gamma, param_nz)

        tensor_list = [data_dy, data_x, data_variance,
                       data_mean, data_gamma, pd_x]

        with tvm.target.cce():
            sch = generic.auto_schedule(pd_x)
